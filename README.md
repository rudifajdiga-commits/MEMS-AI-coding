...
## Features
- **Hybrid 2D/3D Display**: Instant orientation visuals via 2D canvas fallback, with seamless background upgrade to high-poly WebGL if internet is available.
...
- **Rendering Engine**: Three.js (via CDN) with internal 2D Canvas fallback engine.
- **Data Pipeline**: LSM6DSV16X IMU -> Madgwick AHRS Filter (ESP32) -> Quaternions -> WebSocket -> WebGL/Canvas (Browser).
...
4. *Note: The system will start in 2D mode instantly. If your phone has internet access, it will automatically upgrade to the high-poly 3D model in the background.*
...
- **3D Evolution**: From simple Cube -> Humanoid Boxes -> Hybrid 2D Fallback + High-Poly WebGL.
- **Black Screen Fix**: Implemented dual-loader system to handle "No Internet" scenarios when connected to ESP32 SoftAP.
